...
class PageManager:
    ...
    def __init__(self, app: Application.Application):
        ...
        self.app_frame = LabelFrame(
            self.root,
            height=WINDOW_HEIGHT - WINDOW_MARGIN,
            width=WINDOW_WIDTH - WINDOW_MARGIN,
        )
    ...
    def setup_tabs(self):
        ...
        for page in self.pages:
            button = ttk.Button(
                self.tab_frame,
                text=page.name,
                command=lambda index=index: self.switch_page(index),
            )
        ...
        OptionMenu(
            ...
            self.control,
            *list(ClubControls),
            command=self.handle_control_selected
        ).pack(side=LEFT)
    ...
    def handle_control_selected(self, value):
        if value == ClubControls.SELECT_CLUB:
            ...
            self.open_club_selector()
        elif value == ClubControls.EDIT_CLUB:
            self.open_club_selector()
            self.open_subpage(AddClubWizard, self.app.club)
